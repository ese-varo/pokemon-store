...
function Cart () {
  const { state: { cartTotal, cartPokemons, cartCounter }, dispatch } = React.useContext(SessionContext)
    ...
    <Card>
      ...
      <Card.Body>
        ...
        <ListGroup className='cart-list'>
          {cartPokemons.map((pokemon, i) => <CartItem key={`${Date.now()}-${i}`} pokemon={pokemon} />)}
        </ListGroup>
        <p className='mt-3'>
        ...
        <p className='mt-3'>
          <span data-testid='quantity' className='text-muted'>Quantity: </span>
          {cartCounter}
        </p>
        {cartCounter ? (
          <StripeCheckout 
